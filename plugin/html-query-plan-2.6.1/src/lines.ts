...
function drawLines(container: Element) {
    ...
    // Remove any previously rendered SVG to prevent duplicates on re-render
    const prev = root.querySelector('svg');
    if (prev && prev.parentElement === root) {
        root.removeChild(prev);
    }
    // Create a fresh SVG drawing context and attach to root
    let draw: Svg = SVG().addTo(root).size('100%', '100%');

    let clientRect: DOMRect = root.getBoundingClientRect();
 ...
 * @param parent Parent .qp-node element.
 */
function drawLinesForParent(draw: Svg, clientRect: DOMRect, parent: Node) {
 ...
 */
function drawArrowBetweenNodes(draw: Svg, clientRect: DOMRect, parent: Node, child: Node, thickness: number, offset: number) {
 ...
 */
function drawArrow(draw: Svg, to: Point, from: Point, bendX: number, thickness: number, nodeId: string, statementId: string) {
