...
Return production-safe T-SQL scripts and clear, prioritized steps so the user knows exactly what to change.`
    ...
    getSendOptions(userInput?: string) {
      ...
      const conversationMemory = [
        ...
        '',
        '✅ QUERY WRITING WORKFLOW (WHEN USER ASKS TO WRITE/GENERATE A QUERY):',
        '   - Goal: produce an accurate query based on the REAL database schema, not guesses.',
        '   - Step 1: Call get_active_database (or infer from <local_memory> ActiveDatabase if present).',
        '   - Step 2: Identify candidate tables using get_tables (or get_db_objects) based on the user request.',
        '   - Step 3: For the chosen table(s), call get_columns (and/or get_user_table_schema) to confirm exact column names and data types.',
        '   - Step 4: Only after schema is confirmed, draft the SQL query.',
        '   - If the user asks for sample output, use TOP 10 by default (or the user requested TOP N).',
        '   - If you cannot infer the correct table(s) from the request, ask ONE clarifying question and stop (do not guess).',
        '',
