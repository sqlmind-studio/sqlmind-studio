...
`@sqlmindstudio/ui-kit` library provides a set of custom elements or components that can
...
To load the `@sqlmindstudio/ui-kit` library in your project, follow these steps:

1. Install the library using npm or yarn:

   ```bash
   npm install @sqlmindstudio/ui-kit
   ```

2. Import the style and the components:

   ```js
   import "@sqlmindstudio/ui-kit/style.css";

   // Import components individually
   import "@sqlmindstudio/ui-kit/bks-table.js";

   // Or import all components
   import "@sqlmindstudio/ui-kit";
   ```
...
While attributes are always `string`, `@sqlmindstudio/ui-kit` converts them to the
...
`@sqlmindstudio/ui-kit` uses [Vue](https://v2.vuejs.org/) as its main framework. Therefore,
...
`@sqlmindstudio/ui-kit` components emit events in the same way as native HTML elements.
