...
const releaseNotesFile = path.join(projectDir, 'build', 'release-notes.md')
...
module.exports = {
  appId: "studio.sqlmind.desktop",
  productName: "SQLMind Studio",
  ...
  extraResources: [
    ...
    if (!('from' in r)) return true
    const fromPath = String(r.from)
    const resolvedFrom = path.isAbsolute(fromPath) ? fromPath : path.resolve(projectDir, fromPath)
    if (!optionalExtraResources.has(resolvedFrom)) return true
    return fs.existsSync(resolvedFrom)
  }),
  fileAssociations: [
    {
      ext: 'db',
  ...
  mac: {
    ...
    icon: './public/icons/mac/sqlmind-icon.icns',
  ...
  nsis: {
    ...
    uninstallDisplayName: 'SQLMind Studio',
    createDesktopShortcut: true,
    createStartMenuShortcut: true,
  },
  appx: {
    applicationId: "sqlmindstudio",
    publisher: certSubject.replaceAll('"', "&quot;"),
    publisherDisplayName: "SQLMind Studio"
  }
}
