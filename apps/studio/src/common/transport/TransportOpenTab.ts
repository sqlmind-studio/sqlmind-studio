...
export type CoreTabType = 'query' | 'table' | 'table-properties' | 'settings' | 'table-builder' | 'backup' | 'import-export-database' | 'restore' | 'import-table' | 'shell' | 'statistics' | 'executionplan'
...
export interface TransportOpenTab<Context = {}> extends Transport {
  ...
  context: Context
  statsData?: string // For statistics tab
  planXml?: string // For execution plan tab
}
...
export function findTable(obj: TransportOpenTab, tables: TableOrView[]): TableOrView | null {
  const ctx: any = (obj as any)?.context
  if (ctx?.table) {
    return ctx.table as TableOrView
  }
