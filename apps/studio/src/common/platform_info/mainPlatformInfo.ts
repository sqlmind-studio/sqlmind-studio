...
export function mainPlatformInfo(): IPlatformInfo {
  ...
  const updatesDisabled = !!p.env.SQLMIND_DISABLE_UPDATES

  // previous builds of SQLMind Studio required native libs for Oracle,
  ...
  if (p.env.PORTABLE_EXECUTABLE_DIR) {
    userDirectory = join(p.env.PORTABLE_EXECUTABLE_DIR, 'sqlmind_studio_data')
  }
  ...
  function isWaylandMode() {
   ...
   return {
    ...
    appVersion,
    parsedAppVersion,
    // cloudUrl: isDevEnv ? 'https://staging.sqlmindstudio.io' : 'https://app.sqlmindstudio.io',
    // cloudUrl: 'https://app.sqlmindstudio.io',  // Original SQLMind Studio server
    cloudUrl: 'https://api.sqltools.co',  // Your local API server (sqltools-api)
    frontendUrl: 'https://sqltools.co',  // Your local frontend (sqltools_io)
    locale,

    // cloudUrl: isDevEnv ? 'http://localhost:3000' : 'https://app.sqlmindstudio.io'
  }
}
