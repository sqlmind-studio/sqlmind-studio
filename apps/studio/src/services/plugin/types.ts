import { PluginRequestData, PluginResponseData } from "@sqlmindstudio/plugin";
import PluginStoreService from "./web/PluginStoreService";
import rawLog from "@bksLogger";
import type { UtilityConnection } from "@/lib/utility/UtilityConnection";

// Extended request types for internal plugin API methods
interface BaseRequest {
  id: string;
}

interface GetQueryTextRequest extends BaseRequest {
  name: "getQueryText";
  args: void;
}

interface GetQueryTextWithCursorRequest extends BaseRequest {
  name: "getQueryTextWithCursor";
  args: void;
}

interface GetQueryResultsRequest extends BaseRequest {
  name: "getQueryResults";
  args: void;
}

interface ExecuteQueryInTabRequest extends BaseRequest {
  name: "executeQueryInTab";
  args: void;
}

interface InsertTextRequest extends BaseRequest {
  name: "insertText";
  args: {
    text: string;
  };
...
interface InsertTextAtCursorRequest extends BaseRequest {
...
interface SwitchDatabaseRequest extends BaseRequest {
  name: "switchDatabase";
  args: {
    databaseName: string;
  };
...
interface GetTabListRequest extends BaseRequest {
  name: "getTabList";
  args: void;
}

interface SwitchToTabRequest extends BaseRequest {
  name: "switchToTab";
  args: {
    tabId: number;
  };
...
// Extended PluginRequestData type that includes internal methods
export type ExtendedPluginRequestData = 
  | PluginRequestData 
  | GetQueryTextRequest 
  | GetQueryTextWithCursorRequest 
  | GetQueryResultsRequest
  | ExecuteQueryInTabRequest
  | InsertTextRequest
  | InsertTextAtCursorRequest
...
export type ManifestV0 = {
  ...
  /** Requires a premium license (Ultimate edition) to use this plugin */
...
export type WebPluginViewInstance = {
  ...
  context: any; // PluginViewContext from @sqlmindstudio/plugin
}
