...
import SQLMindPluginRepositoryService from "./SQLMindPluginRepositoryService";
...
export type PluginManagerOptions = {
  ...
  appVersion: string;
  /** Function to check if user has a premium license */
  hasLicense?: () => boolean;
}
...
export default class PluginManager {
  ...
  constructor(readonly options: PluginManagerOptions) {
    ...
    // Use SQLMind's bundled plugin repository service instead of GitHub
    this.registry =
      options.registry ||
      new PluginRegistry(new SQLMindPluginRepositoryService());
  }
  ...
  async initialize() {
    ...
    for (const id of PluginManager.PREINSTALLED_PLUGINS) {
      ...
      try {
        ...
        log.warn(`Failed to preinstall plugin "${id}". Skipping.`, e);
      }
    }


    for (const plugin of installedPlugins) {
      if (!this.pluginSettings[plugin.id]?.autoUpdate) continue;
      ...
      try {
        ...
        log.warn(`Failed to auto-update plugin "${plugin.id}". Skipping.`, e);
      }
    }
  }
  ...
  isPluginLoadable(manifest: Manifest): boolean {
    ...
    if (manifest.minAppVersion) {
      const versionCompatible = semver.lte(semver.coerce(manifest.minAppVersion), semver.coerce(this.options.appVersion));
    ...
    if (manifest.requiresLicense && this.options.hasLicense) {
      return this.options.hasLicense();
    }
  ...
  async installPlugin(id: string): Promise<Manifest> {
    ...
    return await this.withPluginLock(id, async () => {
      ...
      if (!this.isPluginLoadable(info.latestRelease.manifest)) {
        if (
          info.latestRelease.manifest.requiresLicense &&
          this.options.hasLicense &&
          !this.options.hasLicense()
        ) {
          throw new NotSupportedPluginError(
            `Plugin "${info.latestRelease.manifest.id}" requires a paid license. ` +
              `Please activate a license to use this plugin.`
          );
        }

        throw new NotSupportedPluginError(
          `Plugin "${info.latestRelease.manifest.id}" is not compatible with app version "${this.options.appVersion}". ` +
            `Please upgrade SQLMind Studio to use this plugin.`
        );
      }
