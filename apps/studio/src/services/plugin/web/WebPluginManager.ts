...
import { PluginNotificationData, PluginViewContext } from "@sqlmindstudio/plugin";
...
export default class WebPluginManager {
  ...
  async uninstall(id: string) {
    ...
    if (!loader) {
      log.warn("Plugin not found (already uninstalled?): " + id);
      return;
    }
    await loader.unload();
    loader.dispose();
  ...
  async reloadPlugin(id: string, manifest?: Manifest) {
    const loader = this.loaders.get(id);
    if (!loader) {
      log.warn("Plugin not found (cannot reload): " + id);
      return;
    }
    await loader.unload();
  ...
  registerIframe(pluginId: string, iframe: HTMLIFrameElement, context: PluginViewContext) {
    ...
    if (!loader) {
      log.warn("Plugin not found (registerIframe ignored): " + pluginId);
      return;
    }
  ...
  unregisterIframe(pluginId: string, iframe: HTMLIFrameElement) {
    const loader = this.loaders.get(pluginId);
    if (!loader) {
      log.warn("Plugin not found (unregisterIframe ignored): " + pluginId);
      return;
    }
  ...
  async notify(pluginId: string, data: PluginNotificationData) {
    const loader = this.loaders.get(pluginId);
    if (!loader) {
      log.warn("Plugin not found (notify ignored): " + pluginId);
      return;
    }
  ...
  pluginOf(pluginId: string) {
    ...
    if (!plugin) {
      log.warn("Plugin not found (pluginOf): " + pluginId);
      return null;
    }
    return plugin;
  }
  ...
  buildUrlFor(pluginId: string, entry: string) {
    const loader = this.loaders.get(pluginId);
    if (!loader) {
      log.warn("Plugin not found (buildUrlFor): " + pluginId);
      return null;
    }
  ...
  onViewRequest(pluginId: string, listener: OnViewRequestListener) {
    const loader = this.loaders.get(pluginId);
    if (!loader) {
      log.warn("Plugin not found (onViewRequest): " + pluginId);
      return () => {};
    }
  ...
  onReady(pluginId: string, fn: Function) {
    const loader = this.loaders.get(pluginId);
    if (!loader) {
      log.warn("Plugin not found (onReady): " + pluginId);
      return () => {};
    }
  ...
  onDispose(pluginId: string, fn: Function) {
    const loader = this.loaders.get(pluginId);
    if (!loader) {
      log.warn("Plugin not found (onDispose): " + pluginId);
      return () => {};
    }
