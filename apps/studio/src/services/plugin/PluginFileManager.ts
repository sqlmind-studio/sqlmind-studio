...
export default class PluginFileManager {
  ...
  async download(
    ...
    const tmpDirectory =
      ...
      path.join(tmpdir(), `sqlmind-plugin-${pluginId}-${Date.now()}`);

    try {
      // Check if this is a bundled plugin (file:// URL)
      const isBundledPlugin = release.sourceArchiveUrl.startsWith('file://');
      
      if (isBundledPlugin) {
        // For bundled plugins, copy directly from the bundled location
        const bundledPath = release.sourceArchiveUrl.replace('file://', '');
        log.debug(
          `Installing bundled plugin "${pluginId}" from "${bundledPath}"...`
        );

        // If we're updating, copy to temp directory first
        if (options.tmp) {
          fs.mkdirSync(tmpDirectory, { recursive: true });
          this.copyDirectory(bundledPath, tmpDirectory);
          return tmpDirectory;
        }
        ...
        // Copy all files from bundled location to final directory
        this.copyDirectory(bundledPath, directory);

        log.debug(`Bundled plugin "${pluginId}" installed successfully`);
        return;
      }

      // For remote plugins, download and extract as before
