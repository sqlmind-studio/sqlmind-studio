...
export const SQLMindPlugin = {
  ...
  buildConnectionString(config: IConnection): string {
    // Safety check: return early if config or connectionType is missing
    if (!config || !config.connectionType) {
    ...
    if (config.connectionType.match(/sqlite|libsql|duckdb/)) {
      ...
      let result = `${config.username || 'user'}@${config.host || 'localhost'}:${config.port || '0'}`
  ...
  simpleConnectionString(config: IConnection): string {
    ...
    let connectionString = `${host}:${port}`;
    ...
    if (config.connectionType.match(/sqlite|libsql|duckdb/)) {
      ...
      return config.url || 'mongodb://localhost';
    } else if (config.connectionType === 'sqlanywhere' && config.sqlAnywhereOptions.mode === 'file') {
...
export type SQLMindPlugin = typeof SQLMindPlugin


export default {
  install(Vue) {
    Vue.prototype.$app = SQLMindPlugin
    Vue.prototype.$bks = SQLMindPlugin
