<template>
  ...
  >
    ...
    >
      ...
      >
        <object-explorer
          v-if="globalSidebarActiveItem === 'tables'"
          @tableSelected="handleTableSelected"
        />
        <core-sidebar
          v-else
        />
...
<script lang="ts">
  ...
  import ObjectExplorer from './sidebar/ObjectExplorer.vue'
  ...
  export default Vue.extend({
    components: { ObjectExplorer, CoreSidebar, CoreTabs, Sidebar, ExportManager, QuickSearch, ProgressBar, LostConnectionModal, RenameDatabaseElementModal, SecondarySidebar, GlobalStatusBar, GlobalSidebar },
    data() {
      /* eslint-disable */
      return {
        split: null,
    ...
    methods: {
      ...
      handleTableSelected(tableName, schema) {
        // Open table data view
        const table = this.$store.state.tables.find(t => t.name === tableName && (!schema || t.schema === schema))
        if (table) {
      ...
      handleToggleOpenSecondarySidebar(force?: boolean) {
        ...
        if (open) {
          ...
          try {
            const containerSize = this.$refs.splitContainer.offsetWidth
            const railPx = 44
            const railSize = (railPx / containerSize) * 100

            const currentSizes = this.split.getSizes()
            const updatedSizes = [...currentSizes]

            updatedSizes[2] = railSize
            updatedSizes[1] = 100 - (updatedSizes[0] + updatedSizes[2])
      ...
      handleSelectGlobalSidebarItem(item) {
        if (this.globalSidebarActiveItem === item) {
          ...
          // Open sidebar if closed, or keep it open if switching items
          if (!this.primarySidebarOpen) {
            this.toggleOpenPrimarySidebar(true)
          }
          // Always set the active item to switch content
