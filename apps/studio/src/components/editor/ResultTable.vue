...
<script type="text/javascript">
  ...
  export default {
    ...
    props: ['result', 'tableHeight', 'query', 'active', 'tab', 'focus', 'binaryEncoding', 'hasExecutionPlan'],
    watch: {
      active() {
    ...
    computed: {
        ...
        const cellMenu = (_e, cell) => {
          return [
            ...copyActionsMenu({
              ...
              vueInstance: this,
              query: this.query,
              hasExecutionPlan: this.hasExecutionPlan,
            }),
            { separator: true },
            filterMenuItem,
      ...
      // Tabulator validates column definition keys and rejects unknown ones like `dataType`.
      // Keep `tableColumns` with our metadata for app logic, but pass a sanitized copy to Tabulator.
      safeTableColumns() {
        try {
          return this.tableColumns.map((col) => {
            const { dataType, ...rest } = col
            return rest
          })
        } catch (_) {
    ...
    methods: {
      initializeTabulator() {
        ...
        this.tabulator = tabulatorForTableData(this.$refs.tabulator, {
          ...
          columns: this.safeTableColumns, //define table columns (sanitized for Tabulator)
