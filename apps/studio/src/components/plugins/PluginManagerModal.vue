<template>
  <portal to="modals">
    <modal
      :name="modalName"
      :class="['vue-dialog', 'sqlmind-modal', 'plugin-manager-modal', { 'plugin-page-open': selectedPlugin }]"
    >
        ...
        <div class="plugin-manager-content">
          <div class="plugin-list-container">
            <div class="description">
              Manage and install plugins in SQLMind Studio.
            </div>
    ...
    pluginManagerStatus: {
      async handler() {
        if (this.pluginManagerStatus === "failed-to-initialize") {
          this.errors = ["Plugin system was not initialized properly. Please restart SQLMind Studio to continue using plugins or report this issue."]
        } else {
          this.errors = null
        }
  ...
  methods: {
    ...
    async buildPluginListData() {
      ...
      for (const { manifest, loadable } of installedPlugins) {
        ...
        // if the plugin is found in the sqlmind-studio-plugins
        if (entry) {
