    ...
    <div class="header">
      ...
      <div class="alert alert-danger" v-if="!plugin.loadable && plugin.installed">
        ...
        <div class="alert-body expand">
          <span>This plugin requires version {{ plugin.minAppVersion }} or newer. Please upgrade your SQLMind Studio or <a href="https://sqltools.co/blog">install</a> a compatible plugin version.</span>
        </div>
      </div>
      <div class="alert alert-danger" v-if="plugin.error">
        ...
        <div class="alert-body expand">
          <span v-if="plugin.error.toString?.().includes('not compatible')">
            {{ plugin.error }}
            Or <a href="https://sqltools.co/blog">install</a> a compatible plugin version.
          </span>
...
export default Vue.extend({
  ...
  computed: {
    repoHref() {
      if (this.plugin?.id === "bks-ai-shell") {
        return "https://github.com/sqlmind-studio/sqlmind_ai_mode";
      }
      return `https://github.com/${this.plugin.repo}`;
    },
    repoLabel() {
      if (this.plugin?.id === "bks-ai-shell") {
        return "ai-mode";
      }
      return this.plugin.repo;
    },
    rawHtmlContent() {
