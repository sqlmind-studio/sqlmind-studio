  ...
  <div class="interface connection-interface">
    <div class="interface-wrap row">
      ...
      <div ref="content" class="connection-main page-content flex-col" id="page-content">
        <div class="small-wrap expand">
          ...
          <template v-if="!config.connectionType">
            <div class="pitch" v-if="!isUltimate">
              ...
              and more!
              <a :href="`${baseFrontendUrl}/pricing`" class="">Upgrade</a>.
            </div>
        ...
        <small class="app-version"><a :href="`${baseFrontendUrl}/download`">SQLMind Studio {{ version
        }}</a></small>
      </div>
    </div>
  ...
  methods: {
    ...
    async submit() {
      ...
      try {
        ...
        // Ensure labelColor is set (fix for status bar color not showing)
        if (!this.config.labelColor) {
          this.config.labelColor = 'default';
        }
