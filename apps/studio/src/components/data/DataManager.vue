...
export default Vue.extend({
  ...
  methods: {
    ...
    mountAndRefresh() {
      ...
      // Use cloud storage when logged into a workspace, local storage otherwise
      const scope = this.workspace.type === 'cloud' ? 'cloud' : 'local'
      console.log('[DataManager] Using storage scope:', scope, 'for workspace:', this.workspace.name)
      DataModules.forEach((module) => {
