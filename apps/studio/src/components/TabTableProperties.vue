...
export default {
  ...
  methods: {
    ...
    async refresh() {
      ...
      try {
        const db = this.tab?.context?.database
        await this.$store.dispatch('updateTableColumns', { table: this.table, database: db })
        console.log("getting primary keys")
        this.primaryKeys = await this.connection.getPrimaryKeys(this.table.name, this.table.schema, db);
        if (this.table.entityType === 'table') {
          console.log("calling getTableProperties")
          this.properties = await this.connection.getTableProperties(this.table.name, this.table.schema, db);
        }
        this.loading = false
      } catch (ex) {
        this.error = ex
    ...
    /** @param {import('@/plugins/SQLMindPlugin').ContextOption} item */
