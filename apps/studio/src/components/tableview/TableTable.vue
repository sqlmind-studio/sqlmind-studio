...
export default Vue.extend({
  ...
  data() {
    return {
      ...
      rawPage: 1,
      initialized: false,
      internalColumnPrefix: "__sqlmind_internal_",
      internalIndexColumn: "__sqlmind_internal_index",
  ...
  computed: {
    ...
    isEmpty() {
      return this.tabulatorBuilt && !this.loading && _.isEmpty(this.data);
    },
    isCassandra() {
    ...
    // to refresh
    tableColumnNames() {
      const tableFromStore = this.tables?.find((t) =>
        t?.name === this.table?.name &&
        t?.schema === this.table?.schema &&
        t?.entityType === this.table?.entityType
      )
      const columns = (this.table?.columns?.length ? this.table.columns : tableFromStore?.columns) || []
    ...
    tableColumns() {
      ...
      const results = columns.map(column => this.createColumnFromProps(column));
  ...
  watch: {
    filters() {
      if (!this.tabulator || !this.tabulatorBuilt) return
      ...
      return [
        ...
        { separator: true },
        {
          ...
          action: () => {
            ...
            const isSecondaryOpen = !!(store && store.state && store.state.sidebar && store.state.sidebar.secondarySidebarOpen)
            if (isSecondaryOpen) {
    ...
    buildPendingInserts() {
      ...
      const inserts = this.pendingChanges.inserts.map((item) => {
        ...
        return {
          table: this.table.name,
          schema: this.table.schema,
          dataset: this.dialectData.requireDataset ? this.effectiveDatabase: null,
          data: [result]
        }
      })
      return inserts
      ...
      const result = new Promise((resolve, reject) => {
        (async () => {
          try {
            ...
            const resultRows = response && Array.isArray(response.result) ? response.result : []
            const resultFields = response && Array.isArray(response.fields) ? response.fields : []
            log.info('selectTop response summary', {
              table: `${this.table?.schema}.${this.table?.name}`,
              database: this.effectiveDatabase,
              rows: resultRows.length,
              fields: resultFields.length,
              sampleRow: resultRows[0] || null,
              sampleRowKeys: resultRows[0] ? Object.keys(resultRows[0]) : [],
              expectedColumnFields: this.tableColumns.map((c) => c.field),
            })

            if (!response || !Array.isArray(response.result) || !Array.isArray(response.fields)) {
              this.error = {
                title: 'Invalid data response',
                message: 'selectTop response did not contain expected result/fields arrays',
              }
              this.data = []
              resolve({ data: [], last_page: true })
              return
            }
            ...
            const responseFieldNames = response.fields.map((f) => f?.name).filter(Boolean)
            const tableFromStore = this.tables?.find((t) =>
              t?.name === this.table?.name &&
              t?.schema === this.table?.schema &&
              t?.entityType === this.table?.entityType
            )
            const effectiveColumns = (this.table?.columns?.length ? this.table.columns : tableFromStore?.columns) || []
            if (_.xor(responseFieldNames, effectiveColumns.map(c => c.columnName)).length > 0) {
              log.debug('table has changed, updating')
              await this.$store.dispatch('updateTableColumns', { table: this.table, database: this.effectiveDatabase })
            }
            ...
            log.info('selectTop mapped table data summary', {
              table: `${this.table?.schema}.${this.table?.name}`,
              database: this.database,
              mappedRows: Array.isArray(data) ? data.length : null,
              mappedSampleRow: Array.isArray(data) ? (data[0] || null) : null,
            })
