        ...
        result.push({
          ...
          handler: async ({item}) => {
            const confirmed = await this.$confirm(
              'Delete Workspace',
              `Are you sure you want to delete "${item.workspace.name}"? This will hide the workspace but preserve all data.`,
              { confirmLabel: 'Delete Workspace' }
            )
            if (confirmed) {
              try {
                // Pass workspace object with cloud_id to backend
                const workspaceToDelete = {
                  ...item.workspace,
                  cloud_id: item.workspace.cloudId || item.workspace.cloud_id
                }
                await item.client.workspaces.delete(workspaceToDelete)
                this.$noty.success('Workspace deleted successfully')
                this.refresh()
              } catch (error) {
                this.$noty.error(`Failed to delete workspace: ${error.message}`)
              }
            }
          }
        })
