 ...
 */
import Vue from "vue";
import TextEditor from "@sqlmindstudio/ui-kit/vue/text-editor"
import {
  ExpandablePath,
  findKeyPosition,
  findValueInfo,
...
import { EditorMarker, LineGutter } from "@sqlmindstudio/ui-kit";
  ...
  watch: {
    ...
    editableRanges() {
      if (this.empty || !this.text) {
        this.partialReadonly.setEditableRanges([])
        return
      }
      this.$nextTick(() => this.partialReadonly.setEditableRanges(this.editableRanges))
    },
  },
  computed: {
    sidebarTitle() {
  ...
  methods: {
    ...
    replaceExtensions(extensions) {
      return [
        ...
        this.partialReadonly.extensions([]),
      ]
    },
