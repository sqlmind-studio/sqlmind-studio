...
export default {
  ...
  computed: {
    ...
    ...mapGetters(['isCloud', 'workspace', 'workspaces']),
    personalWorkspace() {
    ...
    isPersonalConnection() {
      ...
      return this.config.workspaceId === this.personalWorkspace.cloudId || 
             this.config.workspaceId === this.personalWorkspace.id
    },
    isTeamConnection() {
      if (!this.teamWorkspace) return false
      return this.config.workspaceId === this.teamWorkspace.cloudId || 
             this.config.workspaceId === this.teamWorkspace.id
    },
    moveToOptions() {
      // Get folders in the same workspace as the connection
      const sameWorkspaceFolders = this.folders.filter(f => f.workspaceId === this.config.workspaceId)
      
      // Filter out the current folder and map to menu options
      return sameWorkspaceFolders
  ...
  methods: {
    showContextMenu(event) {
      ...
      if (this.isCloud && this.moveToOptions.length > 0) {
        // Add folder-based move options only
        options.push(...[
          {
            type: 'divider'
          },
    ...
    async moveItem({ item, option }) {
      try {
        ...
        // Reload connections to update UI immediately
    ...
    async moveToWorkspace({ item, workspaceId }) {
      try {
        ...
        // Get the actual workspace GUID
        let finalWorkspaceId
        if (isMovingToPersonal) {
          if (!this.personalWorkspace) {
            this.$noty.error('Personal workspace not found')
            return
          }
          finalWorkspaceId = this.personalWorkspace.cloudId
        } else {
          finalWorkspaceId = workspaceId
        }
        
        console.log("moving connection to workspace!", {
