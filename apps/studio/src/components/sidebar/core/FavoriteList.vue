    ...
    <div class="sidebar-list">
      <div class="list-group">
        ...
        >
          ...
          <sidebar-folder
            :title="`Personal (${personalQueries.length})`"
            :expanded-initially="true"
            placeholder="No personal queries"
          >
            <favorite-list-item
              v-for="item in personalQueries"
              :key="item.id"
              :item="item"
          ...
          <sidebar-folder
            :title="`Team (${teamQueries.length})`"
            :expanded-initially="true"
            placeholder="No team queries"
          >
            <favorite-list-item
              v-for="item in teamQueries"
              :key="item.id"
              :item="item"
              :active="isActive(item)"
              :selected="selected === item"
...
export default {
  ...
  mounted() {
    ...
    // Load query folders when component mounts
    this.$store.dispatch("data/queryFolders/load")
  },
  beforeDestroy() {
  ...
  computed: {
    ...
    // Separate queries by folder name (Personal/Team), not by workspace
    personalQueries() {
      // Find the "Personal" folder
      const personalFolder = this.folders.find(f => f.name.toLowerCase().trim() === 'personal')
      if (!personalFolder) return []
      
      // Return queries in the Personal folder
      return this.filteredQueries.filter((q) => q.queryFolderId === personalFolder.id)
    },
    teamQueries() {
      // Find the "Team" folder
      const teamFolder = this.folders.find(f => f.name.toLowerCase().trim() === 'team')
      if (!teamFolder) return []
      
      // Return queries in the Team folder
      return this.filteredQueries.filter((q) => q.queryFolderId === teamFolder.id)
    },
    // Filter folders by workspace
    personalFolders() {
      if (!this.personalWorkspace) return []
      return this.folders.filter((f) => 
        f.workspaceId === this.personalWorkspace.cloudId || 
        f.workspaceId === this.personalWorkspace.id
      )
    },
    teamFolders() {
      if (!this.teamWorkspace) return []
      return this.folders.filter((f) => 
        f.workspaceId === this.teamWorkspace.cloudId || 
        f.workspaceId === this.teamWorkspace.id
      )
    },
    // Personal workspace folders and queries
    personalFoldersWithQueries() {
      return this.personalFolders.map((folder) => {
        return {
          folder,
          queries: this.personalQueries.filter((q) =>
            q.queryFolderId === folder.id
          )
        }
      }).filter(({ queries }) => queries.length > 0)
    },
    personalLonelyQueries() {
      const folderIds = this.personalFolders.map((f) => f.id)
      return this.personalQueries.filter((query) => {
        return !query.queryFolderId || !folderIds.includes(query.queryFolderId)
      })
    },
    // Team workspace folders and queries
    teamFoldersWithQueries() {
      return this.teamFolders.map((folder) => {
        return {
          folder,
          queries: this.teamQueries.filter((q) =>
            q.queryFolderId === folder.id
          )
        }
      }).filter(({ queries }) => queries.length > 0)
    ...
    teamLonelyQueries() {
      const folderIds = this.teamFolders.map((f) => f.id)
      return this.teamQueries.filter((query) => {
        return !query.queryFolderId || !folderIds.includes(query.queryFolderId)
      })
    },
