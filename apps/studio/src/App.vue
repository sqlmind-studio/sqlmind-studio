<template>
  <div class="style-wrapper">
    <div
      class="sqlmind-studio-wrapper"
      :class="{ 'sqlmind-studio-minimal-mode': $store.getters.minimalMode }"
    >
      <titlebar />
      <toolbar />
...
export default Vue.extend({
  name: 'App',
  components: {
    CoreInterface, ConnectionInterface, Titlebar, Toolbar, AutoUpdater, NotificationManager,
  ...
  async mounted() {
    ...
    // Make Vue app globally accessible for plugins
    window.$app = this;
    console.log('[App] Vue app instance set on window.$app');

    // Listen for plugin error notifications
    window.addEventListener('plugin-error-notification', (event) => {
      console.log('[App] Received plugin-error-notification event:', event.detail);
      const { message } = event.detail;
      if (this.$noty && this.$noty.error) {
        console.log('[App] Showing error notification via $noty');
        this.$noty.error(message);
      } else {
        console.error('[App] $noty not available, cannot show notification');
      }
    });
        ...
        const options = {
          ...
          buttons: [
            Noty.button('Buy a License', 'btn btn-flat', () => {
              const frontendUrl = (window as any).platformInfo?.frontendUrl || process.env.VUE_APP_FRONTEND_URL || 'https://sqltools.co'
