...
function getIcon() {
  // For Windows taskbar/window icon, prefer .ico
  if (platformInfo.isWindows) {
    return path.resolve(path.join(__dirname, '..', 'SQLMind.ico'))
  }
  // For macOS, prefer .icns
  if (platformInfo.isMac) {
    return path.resolve(path.join(__dirname, '..', 'public/icons/mac/sqlmind-icon.icns'))
  }
  // Linux and fallback
...
class SQLMindWindow {
  ...
  constructor(protected settings: IGroupedUserSettings, openOptions: OpenOptions) {
    ...
    this.win.webContents.on('will-navigate', (e, url) => {
      ...
      e.preventDefault()
      const u = new URL(url)
      u.searchParams.append('ref', 'sqlmind-studio-app')
  ...
  private async initialize() {
    ...
    // Uncomment the line below to auto-open DevTools on startup
...
export function getActiveWindows(): SQLMindWindow[] {
  return _.filter(windows, 'active')
}

export function buildWindow(settings: IGroupedUserSettings, options?: OpenOptions): void {
  windows.push(new SQLMindWindow(settings, options || {}))
}

export function getCurrentWindow(): SQLMindWindow {
