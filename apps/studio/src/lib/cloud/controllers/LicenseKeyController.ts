...
export class LicenseKeyController {
  ...
  async get(email: string, key: string, installationId = "", platformInfo: IPlatformInfo, deviceId = "", deviceName = "", deviceOS = ""): Promise<ILicenseKey> {
    const params: any = {
      email
    }
    
    // Add device binding parameters if provided
    if (deviceId) {
      params.deviceId = deviceId;
      params.deviceName = deviceName || 'Unknown Device';
      params.deviceOS = deviceOS || platformInfo.platform;
      log.info("Device binding enabled - deviceId:", deviceId);
    }
