    ...
    for (const [rangeId, range] of state.ranges) {
      try {
        ...
        if (
          range.from.line < 0 ||
          range.to.line < 0 ||
          range.from.line >= docLines ||
          range.to.line >= docLines
        ) {
          continue;
        }
        const from =
        ...
        const isInvalidLineRangeError =
          errName === "RangeError" && errMessage.includes("Invalid line number");

        if (!isInvalidLineRangeError) {
