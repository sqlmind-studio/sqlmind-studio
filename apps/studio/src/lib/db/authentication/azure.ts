...
export class AzureAuthService {
  ...
  private async accessToken(): Promise<AuthConfig> {
    ...
    log.debug('Getting sqlmind cloud token');
    const res = await axios.post('https://app.sqlmindstudio.io/api/cloud_tokens') as Response;

    const sqlmindCloudToken = res.data?.cloud_token;
    const authCodeUrlParams = {
      scopes: ['https://database.windows.net/.default', 'offline_access'],
      redirectUri: sqlmindCloudToken.fulfillment_url,
      state: sqlmindCloudToken.id,
      prompt: 'consent'
    };
    ...
    const result = await this.checkStatus(sqlmindCloudToken.url);
    ...
    await axios.put(sqlmindCloudToken.url, { status: 'claimed' });
