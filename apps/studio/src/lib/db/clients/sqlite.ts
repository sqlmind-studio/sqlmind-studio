      ...
      execute: (async (): Promise<QueryResult> => {
        try {
          ...
          if (err.message?.startsWith('no such column')) {
            const nuError = new ClientError(`${err.message} - Check that you only use double quotes (") for identifiers, not strings`, "https://sqltools.co/support/troubleshooting/#no-such-column-x");
            throw nuError
          }
