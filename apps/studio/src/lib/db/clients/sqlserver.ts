...
// Result type that satisfies BaseQueryResult interface
// Note: columns are cast from sql.IColumnMetadata[] at runtime to preserve metadata access
...
export class SQLServerClient extends BasicDatabaseClient<SQLServerResult> {
  ...
  async listTables(filter: FilterOptions): Promise<TableOrView[]> {
    ...
    log.info("🔍 [SQLServer.listTables] Executing query")
    log.info("📝 [SQLServer.listTables] SQL:", sql)
    log.info("🔍 [SQLServer.listTables] Current database context:", this.database?.database)
    
    const { data } = await this.driverExecuteSingle(sql);
    
    log.info("📊 [SQLServer.listTables] Query returned:", {
      rowCount: data.recordset.length,
      sampleRows: data.recordset.slice(0, 5).map(r => `${r.table_schema}.${r.table_name}`)
    })
  ...
  async executeQuery(queryText: string, options: ExecuteOptions = {}) {
    ...
    const { data, rowsAffected, messages } = await this.driverExecuteSingle(queryText, options) as any;
    ...
    // Also create a fake result if there are messages (DBCC, PRINT) even with 0 rows affected
    const hasMessages = messages && messages.length > 0
    const results = !data.recordsets.length && (rowsAffected > 0 || hasMessages) ? [[] as any] : data.recordsets as IRecordSet<any>

    return results.map((result, idx) => {
      const parsed = this.parseRowQueryResult(result, rowsAffected, commands[idx], result?.columns, options.arrayRowMode)
      // attach messages if present so UI can access STATISTICS output
      return Object.assign({}, parsed, { messages })
    })
  }
  ...
  async selectTop(table: string, offset: number, limit: number, orderBy: OrderBy[], filters: string | TableFilter[], schema?: string, selects = ['*']): Promise<TableResult> {
    this.logger().info(`🔍 [selectTop] START - table: ${table}, schema: ${schema}, database: ${this.database?.database}`)
    this.logger().debug("filters", filters)
    let query = await this.selectTopSql(table, offset, limit, orderBy, filters, schema, selects)
    if (this.database?.database) {
      const escapedDb = `[${String(this.database.database).replace(/]/g, ']]')}]`
      query = `USE ${escapedDb};\n${query}`
    }
    this.logger().info(`📝 [selectTop] Query:\n${query}`)

    const result = await this.driverExecuteSingle(query)
    this.logger().info(`📊 [selectTop] Query executed - rows: ${result.data?.recordset?.length || 0}`)
    this.logger().debug(result)
    const fields = this.parseQueryResultColumns(result)
    const rows = await this.serializeQueryResult(result, fields)
    this.logger().info(`✅ [selectTop] Returning ${rows.length} rows`)
  ...
  async getDatabasesWithState(filter?: DatabaseFilterOptions) {
    const databaseFilter = filter ? buildDatabaseFilter(filter, 'name') : '';
    const sql = `
  ...
  */
  ...
  protected async rawExecuteQuery(q: string, options: any): Promise<SQLServerResult & { messages?: string[] }> {
    ...
    const runQuery = async (connection: Request) => {
      ...
      // Best-effort capture of informational messages (STATISTICS IO/TIME)
      const messages: string[] = []
      const listeners: Array<{ event: string, handler: any }> = []
      const safeOn = (ev: string, handler: any) => {
        try {
          if (typeof (connection as any).on === 'function') {
            (connection as any).on(ev, handler)
      ...
      const pushMsg = (m: any) => {
        try {
          const txt = m?.message || m?.toString?.() || String(m)
          if (txt) messages.push(txt)
        } catch { /* ignore */ }
      }
      // mssql/tedious combinations may emit different events; attach to common ones
      safeOn('info', pushMsg)
      safeOn('message', pushMsg)
      safeOn('infoMessage', pushMsg)
      safeOn('errorMessage', pushMsg)

      const detachAll = () => {
        try {
          if (typeof (connection as any).removeListener === 'function') {
            listeners.forEach(l => (connection as any).removeListener(l.event, l.handler))
          }
        } catch { /* ignore */ }
      }
      ...
      // Cast columns to satisfy BaseQueryResult interface while preserving runtime data
      return { connection, data, rowsAffected, columns: columns as any, rows, arrayMode, messages }
    };
  ...
  async getViewCreateScript(view: string, schema?: string) {
    // Build fully qualified name if schema is provided
    const qualifiedName = schema ? `${schema}.${view}` : view;
    const sql = `SELECT OBJECT_DEFINITION (OBJECT_ID('${qualifiedName}')) AS ViewDefinition;`;
  ...
  private async configDatabase(server: IDbConnectionServer, database: IDbConnectionDatabase, signal?: AbortSignal): Promise<any> { // changed to any for now, might need to make some changes
    const dbName = (typeof database?.database === 'string' && database.database.trim().length > 0)
      ? database.database
      : 'master'
  ...
  private genSelectNew(table: string,
    ...
    // When using OFFSET/FETCH, SQL Server requires a valid ORDER BY clause
    // If no orderBy is provided but we're using OFFSET/FETCH, order by first column
    let orderByString = this.genOrderByString(orderBy)
    if (offsetString && (!orderBy || orderBy.length === 0)) {
      // Use (SELECT NULL) as fallback, but when OFFSET is used, order by first select column
      orderByString = selects && selects.length > 0 && selects[0] !== '*' 
        ? `ORDER BY ${this.wrapIdentifier(selects[0])}` 
        : 'ORDER BY 1'
    }
