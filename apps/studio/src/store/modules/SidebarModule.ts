...
export const SidebarModule: Module<State, RootState> = {
  ...
  state: () => ({
    tabs: [
      {
        id: "bks-ai-shell",
        label: "AI Mode",
        url: "plugin://bks-ai-shell/dist/index.html",
      },
      {
        id: "bks-ai-shell-history",
        label: "AI Chat History",
        url: "plugin://bks-ai-shell/dist/index.html?view=history",
      },
      {
        id: "json-viewer",
        label: "JSON Viewer",
    ...
    secondaryActiveTabId: "bks-ai-shell",
  ...
  actions: {
    ...
    setSecondarySidebarOpen(context, open: boolean) {
      // Temporary debug: identify who is forcing the sidebar open/closed
      try {
        // eslint-disable-next-line no-console
        const stack = new Error().stack
        console.log('[SidebarModule] setSecondarySidebarOpen', { open, stack })
      } catch (_) {}
