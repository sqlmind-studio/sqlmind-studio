...
import { CredentialsModule, WSWithClient, extractWorkspaceGuid } from './modules/CredentialsModule'
...
const log = RawLog.scope('store/index')
 ...
 let updateRoutinesRequestSeq = 0
...
const store = new Vuex.Store<State>({
  ...
  getters: {
    ...
    workspaces(_state, getters): IWorkspace[] {
      const workspaces: WSWithClient[] = getters['credentials/workspaces'] || []
  ...
  mutations: {
    ...
    workspaceId(state, id: number) {
      ...
      try {
        const workspaces: WSWithClient[] = (store.getters['credentials/workspaces'] as WSWithClient[]) || []
        const active = workspaces.find(({ workspace }) => workspace.id === state.workspaceId)?.workspace
        const workspaceGuid = extractWorkspaceGuid(active)
        if (workspaceGuid) {
          localStorage.setItem('workspaceId', workspaceGuid)
        }
        sendUserContextToPlugin()
      } catch (err) {
        console.warn('[AI Context] Failed to sync workspaceId to plugin:', err)
      }
    },
  ...
  actions: {
    ...
    updateWindowTitle(context) {
      ...
      let title = config
        ? `${SQLMindPlugin.buildConnectionName(config)} - SQLMind Studio`
        : 'SQLMind Studio'
    ...
    async ensureBackendDatabase(context, database?: string) {
      const targetDatabase = database ?? context.state.database
      if (!targetDatabase) return
      if (!context.state.connection) return

      let databaseForServer = targetDatabase
      if (context.state.connectionType === 'surrealdb') {
        databaseForServer = `${context.state.namespace}::${targetDatabase || ''}`
      }
    ...
    async changeDatabase(context, newDatabase: string) {
      ...
      log.info("🔍 [changeDatabase] BEFORE - Current state:", {
        currentDatabase: context.state.database,
        tablesCount: context.state.tables.length,
        routinesCount: context.state.routines.length,
        sampleTables: context.state.tables.slice(0, 3).map(t => `${t.schema}.${t.name}`)
      })
      ...
      log.info("🔄 [changeDatabase] Sending conn/changeDatabase request for:", databaseForServer)
      await Vue.prototype.$util.send('conn/changeDatabase', { newDatabase: databaseForServer });
      log.info("✅ [changeDatabase] Database switch completed on backend")
      
      context.commit('database', newDatabase)
      log.info("📝 [changeDatabase] Updated state.database to:", newDatabase)
      
      // Clear old tables/routines to prevent showing stale data from previous database
      log.info("🧹 [changeDatabase] Clearing old tables and routines...")
      context.commit('tables', [])
      context.commit('routines', [])
      log.info("✅ [changeDatabase] Cleared - tables:", context.state.tables.length, "routines:", context.state.routines.length)
      
      log.info("🔄 [changeDatabase] Fetching new tables for database:", newDatabase)
      await context.dispatch('updateTables')
      log.info("✅ [changeDatabase] Tables updated - count:", context.state.tables.length)
      
      await context.dispatch('updateDatabaseList')
      await context.dispatch('updateRoutines')
      
      log.info("🔍 [changeDatabase] AFTER - Final state:", {
        currentDatabase: context.state.database,
        tablesCount: context.state.tables.length,
        routinesCount: context.state.routines.length,
        sampleTables: context.state.tables.slice(0, 5).map(t => `${t.schema}.${t.name}`)
      })
    },
    ...
    async updateTableColumns(context, payload: TableOrView | { table: TableOrView, database?: string }) {
      const table = (payload as any)?.table ? (payload as any).table : (payload as TableOrView)
      const database = (payload as any)?.table ? (payload as any).database : undefined
      ...
      try {
        ...
        // Keep the passed-in table reference up-to-date (important for tabs that store a table snapshot)
        try {
    ...
    async updateDatabaseList(context) {
      ...
      try {
        const current = context.state.database
        if (current && Array.isArray(databaseList) && databaseList.length > 0) {
          const match = databaseList.find((db) =>
            typeof db === 'string' && db.toLowerCase() === String(current).toLowerCase()
          )
          if (match && match !== current) {
            context.commit('database', match)
          }
        }
      } catch (_) {}
    },
    ...
    async updateTables(context) {
      ...
      try {
        const schema = null
        await context.dispatch('ensureBackendDatabase')
        log.info("🔍 [updateTables] START - Current database:", context.state.database)
        log.info("🔍 [updateTables] Connection state:", {
          connectionType: context.state.connectionType,
          database: context.state.database,
          hasConnection: !!context.state.connection
        })
        
        context.commit("tablesLoading", "Loading tables...")
        
        log.info("🔄 [updateTables] Calling connection.listTables() for database:", context.state.database)
        const onlyTables = await context.state.connection.listTables(schema);
        log.info("📊 [updateTables] Received tables:", {
          count: onlyTables.length,
          sampleTables: onlyTables.slice(0, 5).map(t => `${t.schema}.${t.name}`)
        })
        ...
        log.info("🔄 [updateTables] Calling connection.listViews()")
        const views = await context.state.connection.listViews(schema);
        log.info("📊 [updateTables] Received views:", views.length)
        views.forEach((v) => {
          v.entityType = 'view'
        })

        log.info("🔄 [updateTables] Calling connection.listMaterializedViews()")
        const materialized = await context.state.connection.listMaterializedViews(schema);
        log.info("📊 [updateTables] Received materialized views:", materialized.length)
        materialized.forEach(v => v.entityType = 'materialized-view')
        const tables = onlyTables.concat(views).concat(materialized)
        
        log.info("📊 [updateTables] Total entities:", {
          tables: onlyTables.length,
          views: views.length,
    ...
    async updateRoutines(context) {
      const requestId = ++updateRoutinesRequestSeq
      const dbAtStart = context.state.database
      if (!dbAtStart) return
      if (!context.state.connection) return

      await context.dispatch('ensureBackendDatabase', dbAtStart)
      const routines: Routine[] = await context.state.connection.listRoutines(null);

      // Ignore stale results (e.g. initial master/default routines overwriting the real DB routines)
      if (requestId !== updateRoutinesRequestSeq) return
      if (context.state.database !== dbAtStart) return
