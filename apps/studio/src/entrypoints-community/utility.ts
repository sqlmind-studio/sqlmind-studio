...
rawLog.info('initializing utility (community)');
...
const pluginManager = new PluginManager({
  ...
  hasLicense: () => {
    return licenseStatus && !licenseStatus.isCommunity;
  },
});
...
async function runHandler(id: string, name: string, args: any) {
  ...
  if ((handlers as any)[name]) {
    return (handlers as any)[name](args)
      .then((data) => {
        replyArgs.data = data;
      })
      .catch((e) => {
...
async function init() {
  ...
  try {
    licenseStatus = await (LicenseHandlers as any)['license/getStatus']();
    log.info('License status loaded:', licenseStatus?.isCommunity ? 'Community' : 'Premium');
  } catch (e) {
    log.error('Error loading license status', e);
    licenseStatus = { isCommunity: true };
  }
